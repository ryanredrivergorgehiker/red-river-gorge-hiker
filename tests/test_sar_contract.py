...
class SarContract(unittest.TestCase):
    def test_authoritative_public_values(self):
        expected = {
            'reportingYear': '2026',
        ...
        self.assertIn("lastUpdated: '2026-08-09'", SAR_DATA)
    ...
    def test_match_meter_can_exceed_the_benchmark(self):
        self.assertIn('sar.matchPercentage > 100', SAR_METER)
        self.assertIn('sar.matchPercentage >= 100', SAR_METER)
        self.assertIn('Math.min(Math.max(sar.matchPercentage, 0), 100)', SAR_METER)
