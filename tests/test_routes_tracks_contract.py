...
class RoutesTracksContractTests(unittest.TestCase):
    ...
    def test_map_has_everyday_controls_and_core_home_view(self):
        ...
        self.assertIn("const homeCenter = L.latLng(37.831, -83.615)", MAP)
    ...
    def test_map_has_mixed_snap_and_offtrail_planning_with_history(self):
        ...
        self.assertIn("event.pointerType === 'mouse'", MAP)
        self.assertIn("window.addEventListener('pointermove', handler, true)", MAP)
        self.assertIn("window.addEventListener('pointerup', handler, true)", MAP)
        self.assertIn("window.addEventListener('pointercancel', handler, true)", MAP)
