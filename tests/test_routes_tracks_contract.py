...
class RoutesTracksContractTests(unittest.TestCase):
    ...
    def test_map_has_mixed_snap_and_offtrail_planning_with_history(self):
        ...
        self.assertIn("window.addEventListener('pointercancel', onPointerCancel, true)", MAP)
        self.assertIn("window.addEventListener('blur', onWindowBlur, true)", MAP)
        self.assertIn("container.getBoundingClientRect()", MAP)
