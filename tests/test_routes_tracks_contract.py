...
class RoutesTracksContractTests(unittest.TestCase):
    ...
    def test_informal_trails_have_public_overpass_failover_and_default_on(self):
        ...
        self.assertIn('new AbortController()', MAP)
        self.assertIn('controller.abort()', MAP)
        self.assertIn('signal: controller.signal', MAP)
        self.assertIn('4000', MAP)
        self.assertNotIn('    await loadInformalTrails();', MAP)
        self.assertIn("window.setTimeout(() => {", MAP)
        self.assertIn("void loadInformalTrails();", MAP)
        self.assertIn('Map ready · loading Community / Informal trails', MAP)
