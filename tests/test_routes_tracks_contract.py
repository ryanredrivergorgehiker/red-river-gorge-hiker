...
class RoutesTracksContractTests(unittest.TestCase):
    ...
    def test_map_has_everyday_controls_and_core_home_view(self):
        ...
        self.assertIn("const homeNorthWest = L.latLng(37.878846, -83.744659)", MAP)
        self.assertIn("const homeZoom = 13", MAP)
        self.assertIn("const homeCenterForViewport = () =>", MAP)
        self.assertIn("northWestPoint.add(L.point(size.x / 2, size.y / 2))", MAP)
        self.assertIn("const setHomeView = () => map.setView(homeCenterForViewport(), homeZoom", MAP)
    ...
    def test_map_has_mixed_snap_and_offtrail_planning_with_history(self):
        ...
        self.assertIn('data-plan-pan', MAP)
        self.assertIn("let planPanMode = false", MAP)
        self.assertIn("setPlanPanMode", MAP)
        self.assertIn("data-plan-pan-mode", MAP)
        self.assertIn("if (planPanMode) return", MAP)
        self.assertIn("activeTool !== 'plan' || planPanMode", MAP)
        self.assertIn("✥ Pan map", MAP)
        self.assertIn("✥ Resume route", MAP)
    ...
    def test_map_layer_logic_matches_outdoor_planning_behavior(self):
        ...
        self.assertIn('data-context-full-opacity="usfs-wilderness" checked', MAP)
        self.assertNotIn('data-opacity="usfs-wilderness"', MAP)
