...
class RoutesTracksContractTests(unittest.TestCase):
    ...
    def test_map_layer_logic_matches_outdoor_planning_behavior(self):
        ...
        self.assertIn("map.setMinZoom(map.getZoom())", MAP)
        self.assertIn("value=\"45\" data-opacity=\"usfs-trails\"", MAP)
        self.assertIn("color: '#5d6387'", MAP)
        self.assertIn("color: '#8a6a46'", MAP)
        self.assertNotIn("setLayerControl('usfs-trails'", MAP)
        self.assertNotIn("setLayerControl('usfs-roads'", MAP)
        self.assertNotIn("setLayerControl('ky-counties'", MAP)
